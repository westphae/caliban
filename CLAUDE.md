...
Required keys: `tempest-token`, `tempest-stationId`, `tempest-deviceId`, `windy-apiKey`. Optional: `windy-stationId` (required iff `windy-v2: true`), `windy-v2` (default `false`), `db-path` (default `wx.DefaultPath()` — see below). The `.gitignore` excludes `*.yml`.
...
- **`windy.Sender` supports two endpoints.** Default is the legacy POST/JSON `pws/update/{key}` URL (Windy is retiring this at end of 2026). `sender.EnableV2(stationID)` switches to GET `api/v2/observation/update` with a Bearer header. **Precip semantics differ:** v2 wants mm-since-local-midnight, legacy wanted mm-in-last-15-min. The daemon picks `obs.LocalDayRainAccumulation` vs `obs.RainAccumulation` based on the `windy-v2` config flag.
